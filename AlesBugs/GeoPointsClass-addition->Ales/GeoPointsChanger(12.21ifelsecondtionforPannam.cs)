...
#endif
                // Before the 'return listSignCoord.Count;' statement
                // Add debug or logging to check the content of listSignCoord
                Console.WriteLine("Before returning listSignCoord: ");
                foreach (var item in listSignCoord)
                {
                    Console.WriteLine($"RoadName: {item.strRoadName}, Lat: {item.dWP_Lat}, Lon: {item.dWP_Lon}");
                }
                // Return listSignCoord.Count as previously done
