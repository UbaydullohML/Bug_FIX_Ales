...
class Program
{
    static void Main(string[] args) // main method is entry point for program
    {
        ...
        var records = csv.GetRecords<GeoPointstest>()
            .Select((record, index) => { record.Index = index + 1; return record; })
            .ToList(); // GetRecords method is used to read and parse csv file into list of objects 
        ...
        var downsampledData = centroids
            .Select((centroid, index) => new GeoPointstest
            {
                Type = records[index].Type,      // Set Type to the original value
                WP_Lat = centroid[0],               // Set WP_Lat to the centroid value
                WP_Lon = centroid[1],               // Set WP_Lon to the centroid value
                WP_Alt = records[index].WP_Alt,  // Set WP_Alt to the original value
                Caution = records[index].Caution, // Set Caution to the original value
